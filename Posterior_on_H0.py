...
for n_events_ in range(n_events):
    ...
    for H_0_ in H0:
        ...
        prior_z_samples[mask] = 0
        #print (len(np.where(prior_z_samples == 0)[0]))
        #sys.exit()
        ...
        tmp = np.log(np.sum(prior_z_samples * integrated_luminosity_distance_term))
...
plt.savefig(str(n_events)+'_events.png')